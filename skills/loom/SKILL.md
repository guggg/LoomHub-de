...
- 你同意後，Loom **讀取本機 clone 的** `schema/skill.schema.json` 與本 spec §3 正文小節規範，據此起草：
  ...
  - 建 `skills/<new-name>/SKILL.md`，填 8 欄位 frontmatter（`version` 起始 `0.1.0`，`owner` = **使用者安裝時親口指定、寫在全域偵測區塊裡的署名**（如 `@Ty`；**不可**用 OS 使用者名稱或自行推斷——那會造成署名錯誤），`updated` = 今日）。若全域區塊裡沒有署名，Loom **先問使用者要用什麼名稱署名**再填，不擅自決定。
  - 產出「用途 / 使用場景 / 使用方式 / Demo / 安裝」正文小節，內容取自當前工作脈絡。
  - **若為收錄外部英文資產**（`source` 有值、原文英文）：依 spec §3.1 語言政策分層處理——`description` / `tags` 中文化（可中英並列，因搜尋只吃這些欄位）、正文 body 保留英文原文、「來源 / 出處」小節寫中文導讀。**此政策以 spec §3.1 為單一真實來源，Loom 引用、不複製。**
