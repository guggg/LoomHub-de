...
- 載入 `index.json`。
- **搜尋**：Fuse.js 對 `name` / `description` / `tags` 做即時模糊比對。
- **篩選**：依 `type` / `category` / `tags`。
- **卡片列表**：顯示 name / description / type / category / tags / version / owner。
- **詳情**：點入渲染該 skill 的 `SKILL.md` 正文（§3.2 小節）+ 一鍵複製安裝指令（§6）。
- **託管**：`vite build` 產出靜態檔，發佈 GitHub Pages；亦可本機開啟。
...
- **bootstrap**：安裝 Loom 後，Loom 自動 `git clone` hub repo 到本機約定位置（如 `~/skillsHub-de/`，實際路徑於 Loom skill 內約定；已存在則 `git pull`），並自動安裝 hub 內指定的相關 skill（§6 機制）。成員無需逐一手動裝。
