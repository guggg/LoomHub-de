...
> 上游：[01-prd](./01-prd.md)（含 Product Brief）、[02-adr](./02-adr/README.md)
...
├── docs/                      # 設計文件（本目錄）
│   ├── 01-prd.md               # Product Brief + PRD（合併）
│   ├── 02-adr/
│   └── 03-spec.md
...
| `upstream` | ⬜ | object | **選填**。結構化上游追蹤，供 `scripts/heartbeat.mjs` 定期偵測上游更新用（見 §7.3）；缺席 = 不追蹤。欄位定義見 `schema/skill.schema.json` |
...
- **收錄英文資產的語言政策（分層中文化，FR-4.3 / ADR-0004）**：搜尋只吃 frontmatter 的 `name` / `description` / `tags`（不搜正文 body），所以外部英文資產收進來時——
  - **`description` / `tags` 必須中文化**（可中英並列，如 `tags: [etl, 資料管線]`）：這是**唯一被搜尋**的欄位，也是解決「英文不好搜」的關鍵；純英文 = 團隊搜不到。
  - **正文 body 保留英文原文**：對上游忠實，利於日後上游更新時（heartbeat 報 §7.3）owner 拿我們的副本與上游做 diff 對帳，也避免全文翻譯的維護成本與誤譯風險。
  - **另加一段中文導讀**：併入「來源 / 出處」小節（§3.2.2），用中文簡述「這是什麼、為何收錄、怎麼用」，解決可讀性。
  - 團隊原創資產本就用中文撰寫，不受此政策影響。
...
§3.2 的三個核心小節（**用途 / 使用場景 / 使用方式**）為**所有 type 共用且必備**（檢核硬項）。核心之上，各 type 的延伸小節**因 type 而異**——不硬套同一組。尤其**末節的「取用方式」分三類**：
...
- **外部連結型（tool）**：資產本身是完全外部、獨立的工具 / 服務，不是要裝進 agent、也不是要複製的文字 → 末節為 **連結 / 前往**（純連結 + 簡述安裝/使用方式，如 `npm install -g openwiki`），**不做 symlink 安裝、也不做複製本文**。
...
| `tool` | （通常無延伸小節，核心三節即足夠說明） | **連結 / 前往**（連到外部專案 + 簡述安裝/使用方式；非安裝、非複製本文） |

此外，**任何 type 若為外部蒐集而來**（`source` 有值），建議補 **來源 / 出處**（原始連結、原作者、授權、我們改了什麼 / 為何收錄）。**若原文為英文**，此小節同時充當**中文導讀**——用中文簡述「這是什麼、為何收錄、怎麼用」，正文其餘 body 保留英文原文（見 §3.1 語言政策）。
...
| `tool` | 完全外部、獨立的工具 / CLI / app / 服務——非可安裝進 agent 的內容，也非可複製的文字範本，僅以連結 + 簡述收錄 |
...
1. **push 觸發部署（建議）**：GitHub Pages Action 綁定 repo，push 到主線即自動跑 `build-index` + `vite build` + 發佈。這是**部署管線、非檢核閘門**，與信任制不衝突。
...
- **託管**：`vite build` 產出靜態檔，依 ADR-0005 發佈 GitHub Pages；亦可本機開啟。
...
#### 7.3.1 `scripts/heartbeat.mjs`（上游 heartbeat，方向與 check-updates 不同）

`check-updates.mjs` 管的是 **hub ↔本機** 這個方向（本機落後/超前 hub 多少）。`heartbeat.mjs`
管的是另一個方向 **上游 ↔ hub**：外部蒐集而來的資產（frontmatter 有 `upstream` 欄位）的原始
專案有新版本時，hub 要主動知道。

- 定期（週排程 + 手動 `workflow_dispatch`）掃描所有帶 `upstream` 的資產，MVP 只支援
  `upstream.type: github` + `upstream.track: release`，比對 `upstream.ref` 與上游最新 Release
  tag；其餘 type/track 或查詢失敗一律 **gracefully skip + warn，不中斷整輪**。
- **只偵測 + 通知，絕不自動套用**：偵測到落差就開一則 GitHub issue 通知資產的 `owner`（標記
  `heartbeat` / `upstream-update`，body 內嵌去重界標避免重開），contributor 跟進後手動更新
  `upstream.ref` + `upstream.checked_at` 並關閉 issue。
- CI 定義於 `.github/workflows/heartbeat.yml`，與 `deploy.yml` 分開；同樣是本 repo 的
  advisory 風格（純通知，非 CI gate）。

---

## 8. Guideline（貢獻指南）

放 `README.md`（已採用此案，未另立 `CONTRIBUTING.md`），涵蓋：
...
4.5. Agent **問使用者署名**：「你之後在 LoomHub-de 分享資產時想用什麼名稱署名？（如 `@Ty`）」——**必須先問、等使用者回答**，記為 `<OWNER_HANDLE>`。**嚴禁**用 OS 使用者名稱、`git config user.name` 或任何自行推斷值代替（這正是署名錯誤的來源，例如把 `@Ty` 誤寫成 `@cfh00585519`）。此值會寫進全域偵測區塊，成為 Loom 起草時 `owner` 欄的單一真實來源。
5. 對每個選定的廠商，agent 把下方「全域偵測區塊」注入該廠商的全域檔案，採**用界標（delimiter）冪等合併**：
   ...
   - 把區塊內的 `{{REPO_PATH}}` 換成步驟 2/3 的實際路徑、`{{OWNER_HANDLE}}` 換成步驟 4.5 使用者親口指定的署名。
...
**全域偵測區塊的確切文字**：單一真實來源見
**[`docs/templates/global-detection-block.md`](./templates/global-detection-block.md)**（已與使用者
定案，只填 `{{REPO_PATH}}`，不改寫用語）。`README.md` 的「給 AI Agent 的安裝指示」一節引用同一份，
不要各自複製維護。
...
- 開通路徑：`使用者對 agent 說「裝 LoomHub-de」→ agent 問 clone 路徑 → 跑 install.sh → agent 問廠商 → agent 問署名 → agent 說明並徵求同意後注入全域偵測區塊（含署名）→ agent 給導覽介紹 → 立即可用、可直接 push 貢獻`。
...
- 使用者同意後，Loom **讀取本機已 clone 的 hub repo** 的 `schema/skill.schema.json` 與正文小節規範（本文件 §3），據此起草：
  - 建立 `skills/<new-name>/SKILL.md`，填入 8 欄位 frontmatter（`version` 起始 `0.1.0`，`owner` = **使用者於安裝時親口指定、寫在全域偵測區塊裡的署名**（如 `@Ty`），**不可**改用 OS 使用者名稱或自行推斷；若區塊中缺此署名，Loom 應先問使用者要用什麼名稱署名，再填）。
