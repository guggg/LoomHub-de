...
§3.2 的三個核心小節（**用途 / 使用場景 / 使用方式**）為**所有 type 共用且必備**（檢核硬項）。但不同 type 的自然內容不同，故各 type **另建議**補上以下小節，讓瀏覽者更快看懂怎麼用（建議、非硬擋；由 Loom 起草時依 type 帶入對應模板）：

| type | 核心三節之外，建議補上 |
|---|---|
| `skill` | **Demo / 範例**（互動示例）、**安裝 / Install**（三家指令） |
| `prompt` | **變數 / 參數**（可填的欄位與說明）、**範例輸出**（貼一段實際輸入 → 輸出）、**模型建議**（適用哪家 / 溫度等，若有） |
| `mcp-server` | **提供的工具 / 資源**（這台 server 掛上後 agent 多了哪些 tool）、**設定 / Config**（連線設定、env、金鑰需求）、**安裝 / 啟動**指令 |
| `workflow` | **步驟總覽**（各階段在做什麼、輸入輸出）、**前置條件**（需先具備什麼）、**Demo / 範例**（跑一遍的樣子） |

此外，**任何 type 若為外部蒐集而來**，建議補：

| 情境 | 建議補上 |
|---|---|
| 外部蒐集/改編（`source` 有值） | **來源 / 出處**（原始連結、原作者、授權，以及「我們改了什麼 / 為何收錄」） |

> 核心三節之外的小節皆為 markdown，模板一律照渲染（含 §3.2.1 demo 區塊），不需為 type 寫特別的頁面邏輯——「模板 type-agnostic、內容依 type 而異」。這些建議小節寫進 guideline（§8）與 Loom 的起草模板（§9.2），供貢獻者參照。
