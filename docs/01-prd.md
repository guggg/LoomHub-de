...
| # | 議題 | 現況 / 待決 | 對應 |
|---|---|---|---|
| Q1 | 分類法：`type` 與 `category` 兩維度的確切白名單 | 已定：type=skill/prompt/mcp-server/workflow（RAG/知識庫歸 skill 或 workflow + tags）；category 採通用活動分類=requirements/design/development/testing/ops/docs/research/general（領域字眼放 tags） | FR-1.2 / FR-1.3，Spec §4 |
