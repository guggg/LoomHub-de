...
- **FR-7.12｜全域主動偵測區塊（持續性偵測機制）**：安裝流程需將一段**固定文字**（定義於 Spec §9.0）注入使用者所選各廠商的**全域**指令檔（`~/.claude/CLAUDE.md` / `~/.codex/AGENTS.md` / `~/.gemini/GEMINI.md`——每個 session、任何專案下都會載入，區別於 FR-7.1 只在 agent 恰好讀到相關工作時才觸發的偵測）：
